...
EVT_HANDLER_MASK(PokeAutoStartFireRed, "Start PokeAuto FireRed starter hunt", CMDEN_GBA)
{
    pokeauto::StartFireRedStarterHunt();
}

EVT_HANDLER(PokeAutoStop, "Stop PokeAuto")
{
    pokeauto::Stop();
}

EVT_HANDLER(PokeAutoStatus, "Show PokeAuto status")
{
    wxMessageBox(wxString::FromUTF8(pokeauto::GetStatusText().c_str()),
                 _("PokeAuto status"), wxOK | wxICON_INFORMATION, this);
}
